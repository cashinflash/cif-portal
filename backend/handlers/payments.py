...
def get_loan_summary(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ── TEMP PROBE: register chain WITHOUT service Bearer ──
    # Vergent's reply 2026-05-30: "All other endpoints should be
    # available for use if pointing to the correct API URL."
    # Previous probe sent service Bearer alongside x-api-key on
    # update/sync — got 403. The docs say bearer is required only
    # IF app is already attached to a customer. Ours wasn't, so the
    # bearer may have actually been the PROBLEM (Vergent treating
    # service identity as wrong context for anonymous applicant
    # actions). Re-test the full chain with x-api-key ONLY.
    # If step 4 (register) returns a token, step 5 charges $0.01
    # using that customer-scoped bearer. Token logging is masked.
    try:
        creds = _get_creds() or {}
        xapikey = creds.get("xApiKey") or creds.get("apiKey") or ""
        h_anon = {"x-api-key": xapikey, "Content-Type": "application/json"}

        # Step 1: create application (anonymous — no Bearer)
        s1, p1, raw1 = _http(f"{APIM_BASE}/api/application", "POST",
                             body={}, headers=h_anon, timeout=20)
        log.info("regchain-probe 1=create status=%s body=%s",
                 s1, (raw1 or "")[:400])
        ...
        if not app_id:
            log.warning("regchain-probe step1 no applicationId, aborting")
        else:
            ...
            applicant_data = {
                ...
                "primaryApplicant": {
                    ...
                    "heardAboutId": 0,
                    "bank": {
                        "localId": "", "name": "", "routingNumber": "",
                        "accountNumber": "", "type": 0,
                        "currentBalance": 0, "doYouHaveBankAccount": 0,
                        "accountOpenDate": None, "isPrimary": False,
                        "numNSFs": 0, "createdDateTime": None,
                    },
                    "identification": {
                        "localId": "", "ssn": "", "stateIssued": "",
                        "idNumber": "", "expirationDate": None,
                    },
                    "income": {
                        "sourceType": 0, "frequency": 0, "dayOfWeek": 0,
                        "dayOfMonth1": 0, "isMonthlyLastDay": 0,
                        "dayOfMonth2": 0, "isDayOfMonth2Lastday": 0,
                        "weekOfMonth": 0, "sameDaySameWeek": 0,
                        "lastPayDate": None, "nextPayDate": None,
                        "isDirectDeposit": False,
                        "beforeTaxAmount": 0, "netPayAmount": 0,
                        "afterTaxAmount": 0, "payFrequencyId": 0,
                    },
                    "employment": {
                        "localId": "", "employerName": "",
                        "contactPhoneNumber": "",
                        "contactPhoneNumberExtension": "",
                        "employerAddress": "", "title": "", "shift": "",
                        "dateEmployed": None, "years": 0, "months": 0,
                        "createdDateTime": None,
                    },
                    "loanRequestInformation": {
                        "amountRequested": 0, "purposeOfLoanId": None,
                    },
                },
            }
            ...
            if isinstance(p4, dict):
                for k in ("token", "Token", "jwt", "Jwt", "bearer", "Bearer"):
                    if p4.get(k):
                        customer_token = p4[k]; break
            log.info("regchain-probe 4=register status=%s token_present=%s "
                     "body=%s",
                     s4, bool(customer_token),
                     "[token-masked]" if customer_token else (raw4 or "")[:500])

            # Step 5: if we got a token, USE IT to charge $0.01.
            if customer_token:
                charge_url = (f"{APIM_BASE}/api/CustomerPortal/Loans/Payments"
                              f"/CreditCardPayment")
                charge_body = {"loanId": 4830592, "paymentId": 237669,
                               "amountDue": 0.01, "isInRescindPeriod": False,
                               "authCode": None}
                charge_h = {"x-api-key": xapikey,
                            "Authorization": f"Bearer {customer_token}",
                            "Content-Type": "application/json"}
                s5, _p5, raw5 = _http(charge_url, "POST", body=charge_body,
                                      headers=charge_h, timeout=30)
                log.info("regchain-probe 5=charge-with-customer-bearer "
                         "status=%s body=%s",
                         s5, (raw5 or "")[:700])
    except Exception as _exc:
        log.warning("regchain-probe error: %s", _exc)
    # ── END TEMP PROBE ──
