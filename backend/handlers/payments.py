...
def _validate_future_date(raw: Any) -> Optional[str]:
    ...
    portal sends for a scheduled card payment) IF it's a valid FUTURE
    date, else None.

    Accepts 'YYYY-MM-DD' (the <input type=date> wire format) or a longer
    ISO timestamp (we read the leading date part). Rejects today and the
    past — "pay now" goes through /charge, not here — and caps the
    horizon at 90 days out to bound fat-finger far-future schedules.
    """
    ...
    s = str(raw).strip()
    try:
        parsed_date = datetime.strptime(s[:10], "%Y-%m-%d").date()
...
def _extract_cardsafe_token_str(raw: str) -> Optional[str]:
    """Pull the token out of CardSafe's XML/string response (mirrors the
    portal's Instant Funding extractor)."""
    ...
    def _ok(s: str) -> bool:
        s = (s or "").strip()
        return (len(s) >= 6 and bool(re.match(r"^[A-Za-z0-9\-]+$", s))
                and s.lower() not in ("success", "ok", "true", "false"))

    try:
        root = ET.fromstring(raw)
    except Exception:
        return None
    cands = ("cardsafeid", "token", "cardtoken", "cardid",
        ...
        if val.startswith("<"):
            try:
                ...
                for sub in inner.iter():
                    st = sub.tag.split("}", 1)[-1].lower() if "}" in sub.tag else sub.tag.lower()
                    if st in cands and sub.text and _ok(sub.text):
...
def _vergent_save_card(*, cid: str, card_ref: str, pan: str,
                       cardholder_name: str, last4: str,
...
def post_card(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ok, status, _snippet = _vergent_save_card(
        cid=cid, card_ref=token, pan=pan,
...
def lambda_handler(event: Dict[str, Any], _context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if path.endswith("/my-payment/schedule") and method == "POST":
            return post_schedule(event)
