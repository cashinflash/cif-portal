...
def get_loan_summary(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ── TEMP PROBE: CreditCardPayment customer-identity variants ──
    # Vergent says CreditCardPayment + our existing auth is all we
    # need (AuthenticateCognito was wrong path — built for one
    # customer's specific use). But our service bearer fails the
    # JWT-based customer lookup. Test how Vergent expects us to
    # identify the customer via:
    #   A) X-Customer-Id header
    #   B) X-Mobile-Profile-Id header
    #   C) customerId in URL query
    #   D) NO Bearer (x-api-key alone) + customerId in body
    #   E) X-On-Behalf-Of header (common partner-API pattern)
    # amountDue=0 ensures the handler validates auth before the
    # charge logic runs — no money moves on these probes.
    try:
        creds = _get_creds() or {}
        xapikey = creds.get("xApiKey") or creds.get("apiKey") or ""
        apim_tok = _get_apim_token() or ""
        cust_id = 601488  # Harut
        loan_id = 4830592
        card_id = 237669
        pay_url = (f"{APIM_BASE}/api/CustomerPortal/Loans/Payments"
                   f"/CreditCardPayment")
        base_body = {"loanId": loan_id, "paymentId": card_id,
                     "amountDue": 0.0, "isInRescindPeriod": False,
                     "authCode": None}
        h_full = {"x-api-key": xapikey,
                  "Authorization": f"Bearer {apim_tok}",
                  "Content-Type": "application/json"}
        ...
        variants = [
            ("A-X-Customer-Id-header",
             pay_url, base_body,
             {**h_full, "X-Customer-Id": str(cust_id),
              "x-customer-id": str(cust_id)}),
            ("B-X-Mobile-Profile-Id-header",
             pay_url, base_body,
             {**h_full, "X-Mobile-Profile-Id": "5926",
              "x-mobile-profile-id": "5926"}),
            ("C-customerId-in-url",
             pay_url + f"?customerId={cust_id}", base_body, h_full),
            ("D-no-bearer-x-api-key-only",
             pay_url,
             {**base_body, "customerId": cust_id},
             {"x-api-key": xapikey, "Content-Type": "application/json"}),
            ("E-X-On-Behalf-Of-header",
             pay_url, base_body,
             {**h_full, "X-On-Behalf-Of": str(cust_id)}),
            ("F-mobileProfileId-in-url",
             pay_url + "?mobileProfileId=5926", base_body, h_full),
        ]
        for name, url, body, headers in variants:
            s, _p, raw = _http(url, "POST", body=body,
            ...
            log.info("custid-probe %s status=%s err=%s body_head=%s",
                     name, s, err_type, head[:300])
    except Exception as _exc:
        log.warning("custid-probe error: %s", _exc)
    # ── END TEMP PROBE ──
